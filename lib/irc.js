...
var parseIrcMessage = require('irc-message').parseMessage;
var net  = require('net');
var tls  = require('tls');
...
function parseMessage(line, stripColors) { // {{{
    ...
    parsedMsg = parseIrcMessage(line);
  
    if ( match = parsedMsg.prefix.match(/^([_a-zA-Z0-9\[\]\\`^{}|-]*)(!([^@]+)@(.*))?$/) ) {
        message.nick = match[1];
        message.user = match[3];
        message.host = match[4];
    }
    else {
        message.server = parsedMsg.prefix;
    }

    message.command = message.rawCommand = parsedMsg.command;
    ...
    message.args = parsedMsg.params.slice();
